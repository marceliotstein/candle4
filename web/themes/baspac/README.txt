...
=====================
Introduction to BasPac
=====================

BasPac boasts a clean HTML5 structure with extensible CSS classes and IDs for
...
BasPac's goal is to provide themers the building blocks needed to get their
designs up and running quickly and simply.

BasPac is perfect if you want a simple, smart, and flexible theme starter.
...
BasPac utilizes SASS for adaptive grids and layouts and general structure of the
...
BasPac is meant to be YOUR theme. Follow one of the two methods below and you can
rename 'baspac' to another name like 'mytheme'. You're also welcome to keep
'baspac'.

1. DRUSH: The provided drush install script will duplicate the baspac theme
   folder and rename all appropriate files and content to the new theme name you
   provide.

  - Download and enable the baspac theme: $ drush en baspac
  - Set the theme as default $ drush config-set system.theme default baspac
    or copy the includes/baspac.drush.inc into your .drush folder.
  - Run the provided drush install script: $ drush baspac-install
  ...
  - At this time, you can uninstall baspac: $drush pmu baspac
...
2. MANUAL: To manually change the name of the theme follow these steps BEFORE
  ...
  - Rename baspac.info.yml to mytheme.info.yml
  - edit baspac.info.yml and change the name, description, project (can be
    deleted), and change all other instances of 'baspac' to 'mytheme'
  - Rename baspac.libraries.yml to mytheme.libraries.yml
  - Rename baspac.theme to mytheme.theme
  - in baspac.theme, change each instance of 'baspac' to 'mytheme'
  - Rename config/schema/baspac.schema.yml to mytheme.schema.yml
  - Rename each file in config/install from block.block.baspac_tools.yml (for
    example) to block.block.mytheme_tools.yml
  - Every file in config/install, change each instance of 'baspac' to
    'mytheme'
  - In js/source/scripts.js, change each instance of 'baspac' to 'mytheme'
  - In theme-settings.php, change each instance of 'baspac' to 'mytheme'
  - In templates/html.html.twig, change each instance of 'baspac' to 'mytheme'
  - In templates/menu-local-tasks.html.twig, change each instance of 'baspac' to
    'mytheme'
  - In templates/status-messages.html.twig, change each instance of 'baspac' to
    'mytheme'

  When renaming, remember the following:
  - Do not simply replace every instance of 'baspac' in every file in the theme.
    Most of BasPac's dependencies use the word 'baspac' somewhere and renaming
    these instances will cause BasPac to break in unpredictable ways.
  ...
  - If you don't bother renaming BasPac in the above locations, be advised that
    you will run into conflicts with other versions of BasPac on your site. If
    your site uses more than one theme based on BasPac, make sure at least one of
    the themes has been renamed properly!


============================
How to compile SASS in BasPac
============================

To use SASS and automatically compile it within your theme, please refer to "How
to Use Grunt with BasPac" in the documentation below.
...
- build/scripts.js & source/scripts.js
  ...
  folder. See comments in baspac.libraries.yml file to enable the starter
  scripts.js file.
...
The layout used in BasPac is fairly similar to the Holy Grail method. It has been
...
This is how the page template is buit in BasPac, and it works in fluid and fixed
layout. Refer to the notes in layout.sass to see how to modify the layout.


===========================
How to Use Grunt with BasPac
===========================
...
Once Node.JS is installed, go to the root folder of BasPac and install your Grunt
packages:
...
Once installed, cd to the root folder of BasPac and run Grunt via the command
line:
...
Once you start using BasPac, you will massively change it until you reach the
point where it has nothing to do with BasPac anymore. Unlike Zen, BasPac is not
...
version of BasPac comes out. Always see BasPac as a STARTER, and as soon as you
start using it, it is not BasPac anymore, but your own theme.

If you didn't rename your theme, but you don't want to be notified when BasPac
has a new version by the update module, simply delete "project = "baspac" in
baspac.info.yml.
...
Thanks for using BasPac, and remember to use the issue queue in drupal.org for
any questions or bug reports:

http://drupal.org/project/issues/baspac
